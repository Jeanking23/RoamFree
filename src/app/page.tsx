      ...
      <section id="stays-section" className="container mx-auto px-4">
        ...
        {isLoading ? (
          <div className="flex gap-6 overflow-x-auto pb-4 md:grid md:grid-cols-2 lg:grid-cols-3 md:overflow-x-visible no-scrollbar">
            {[...Array(3)].map((_, i) => (
              <Card key={i} className="overflow-hidden flex flex-col group rounded-lg w-[80vw] sm:w-[45vw] md:w-full flex-shrink-0">
                ...
                <CardFooter className="p-3 bg-muted/30 rounded-b-lg"><Skeleton className="h-10 w-full" /></CardFooter>
              </Card>
            ))}
          </div>
        ) : (
